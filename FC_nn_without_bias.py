...
class FC_withou_b(my_metrics):
    ...
    def initialization(self, dimensions: np.ndarray) -> dict:
        ...
        for l in range(1, self.N_layers+1):  # loop on the number of layers
            parameters['W' +
                       str(l)] = np.random.randn(dimensions[l], dimensions[l - 1])
    ...
    def forward_propagation(self, X: np.ndarray, parameters: dict) -> dict:
        ...
        '''
        activations = {
            'A0': X}  # the 0th activation function output correponds to the input X
        for l in range(1, self.N_layers+1):  # loop on the layers
            # Z_l has a shape (n_unit_l, n_sample)
            # Z_l = W_l * A_l-1 + b_l
            Z = parameters['W' + str(l)].dot(activations['A' + str(l - 1)])
    ...
    def back_propagation(self, y: np.ndarray, activations: dict, parameters: dict) -> dict:
        ...
        gradients = {}
        # loop on the layers from the last to the first
        for l in reversed(range(1, self.N_layers + 1)):
            gradients['dW' + str(l)] = (1 / m) * \
                np.dot(dZ, activations['A' + str(l-1)].T)
    ...
    def update(self, gradients: dict, parameters: dict) -> dict:
        ...
        for l in range(1, self.N_layers + 1):  # loop on the layers
            ...
            parameters['W' + str(l)] = parameters['W' + str(l)] - \
                self.learning_rate * gradients['dW' + str(l)]
    ...
    def early_stop_inf(self, f_true_test: list, stop: int) -> int:
        ...
        if f_true_test[-1] < f_true_test[-2-stop]:
            stop += 1
            print('new test:', f_true_test[-1],
                  ' < ', f_true_test[-2-stop], 'old test')
        else:
            stop = 0
        return stop
    ...
    def train_nn(self, X: np.ndarray, y: np.ndarray, hidden_layers: np.ndarray,
        ...
        # total number of samples in test
        ...
        for i in range(self.n_epochs):  # loop on the number of epochs
            ...
            ind, N_batch = self.set_batch(
                self.N_samples_train, self.batch_size)
            if i == 0:
            ...
            for b in range(N_batch):  # loop on the batches
                ...
                activations = self.forward_propagation(
                    X[:, ind_b], parameters)  # forward propagates
                gradients = self.back_propagation(
                    y[:, ind_b], activations, parameters)  # computes the gradients
                # updates the weights and biaises
                parameters = self.update(gradients, parameters)
            # train save results
            # loss
            activations = self.forward_propagation(
                X, parameters)  # forward propagates
            train_loss.append(self.my_log_loss(
                y.flatten(), activations['A' + str(self.N_layers)].flatten()))
            # accuracy
            y_pred = self.predict(X, parameters)
            train_acc.append(self.my_accuracy_score(
                y.flatten(), y_pred.flatten()))
            # test save results
            # loss
            activations_test = self.forward_propagation(X_test, parameters)
            test_loss.append(self.my_log_loss(
                y_test.flatten(), activations_test['A' + str(self.N_layers)].flatten()))
            # accuracy
            y_pred_test = self.predict(X_test, parameters)
            test_acc.append(self.my_accuracy_score(
